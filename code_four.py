...
def getWideDeepModel(linear_inputs, dnn_inputs, y_train, linear_epochs=200, dnn_epochs=500, combined_epochs=800):
    ...
    dnn_model.fit(dnn_inputs, y_train, epochs=dnn_epochs, batch_size=32)
    ...
    combined_model.fit([linear_inputs, dnn_inputs], y_train, epochs=combined_epochs, batch_size=32)
    return combined_model

# %% read data
df = pd.read_excel(r'df_four.xlsx')
...
# %% Model
#### GBM
other_params = {'n_estimators': 400, 'learning_rate': 0.09, 'max_depth': 3, \
                'random_state': 42}
...
#### CatBoost
best_params_cbt = {'depth': 4, 'iterations': 500, 'learning_rate': 0.2, 'random_state': 42}
...
best_hidden_nums, best_drop_out, best_epochs = 100, 0.1, 200
...
max_epochs = 200
batch_size = 64
best_tabnet = TabNetRegressor(cat_idxs=[0,5,6,7,8,9,10], cat_dims=[7,2,2,2,2,2,2], seed=42,\
                        n_d=best_nd, n_a=best_na, n_steps=best_n_steps, n_independent=1, n_shared=1, \
                        momentum=0.4, scheduler_params={'lr': 0.01})
best_tabnet.fit(
...
#### Wide&Deep
catCols = ['daily_dose', 'C1.5h>C0.5h', 'C9h>C0.5h', 'C4h>C1.5h', 'C9h>C1.5h', 'C9h>C4h', 'C4h>C0.5h']
conCols = ['C0.5h', 'C1.5h', 'C4h', 'C9h', 'ALT', 'AST', 'cr']

X_train_cat = X_train[catCols]
X_test_cat = X_test[catCols]
X_train_con = X_train[conCols]
X_test_con = X_test[conCols]
best_com_epochs = 1000
