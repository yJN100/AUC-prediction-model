...
def getWideDeepModel(linear_inputs, dnn_inputs, y_train, linear_epochs=400, dnn_epochs=600, combined_epochs=800):
    ...
    linear_model.fit(linear_inputs, y_train, epochs=linear_epochs, batch_size=16)  # loss=965.6911
    ...
    combined_model.fit([linear_inputs, dnn_inputs], y_train, epochs=combined_epochs, batch_size=16)
    return combined_model

# %% read data
df = pd.read_excel(r'df_two.xlsx')
...
# %% Model
#### GBM
other_params = other_params = {'n_estimators': 100, 'learning_rate': 0.1, 'max_depth': 6, 'random_state': 42}
...
#### CatBoost
best_params_cbt = {'depth': 3, 'iterations': 300, 'learning_rate': 0.03, 'random_state': 30}
...
best_hidden_nums, best_drop_out, best_epochs = 100, 0.1, 300
...
#### Wide&Deep
catCols = ['daily_dose', 'C9h>C1.5h']
conCols = ['C1.5h', 'C9h', 'ALT', 'AST', 'cr']
